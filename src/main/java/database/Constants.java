...
public class Constants {
    ...
    public static Map<String, List<String>> getRolesRights() {
        Map<String, List<String>> rolesRight = new HashMap<>();
        for (String role : Roles.ROLES) {
            rolesRight.put(role, new ArrayList<>());
        }

        rolesRight.get(ADMINISTRATOR).addAll(Arrays.asList(RIGHTS));

        rolesRight.get(EMPLOYEE).addAll(Arrays.asList(CREATE_BOOK, DELETE_BOOK, UPDATE_BOOK, SELL_BOOK));

        rolesRight.get(CUSTOMER).addAll(Arrays.asList(SELL_BOOK, BUY_BOOK, RETURN_BOOK));
